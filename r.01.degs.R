...
# 加载表达矩阵数据
exp <- read.csv(file ="../00_rawdata/01_GEO/GSE76427.txt",
                sep="\t",row.names=1)
# 加载分组变量
group_list <- read.csv(file = "../00_rawdata/01_GEO/GSE76427.group.txt",
                  sep="\t")
# sample <- sample %>% column_to_rownames("sample")
...
# 调整差异比较的方向，Normal为-1
contrast.mat = makeContrasts(contrasts="tumor-normal", levels=design) #调整差异比较的方向，Normal为-1
contrast.mat
...
if(F){
  ...
  write.csv(DEG_write,file = "01.DEG_all_GSE13355.csv")
  sig_diff_write <- sig_diff[,-c(2,3,6)]
  write.csv(sig_diff_write, file = "01.DEG_sig_GSE13355.csv")
}
...
mat <- as.matrix(mat)
heatmap_plot <- 
  HeatmapAnnotation(Group = group1$group, col = list(Group = c("tumor" = "#FF0000", "normal" = "#0000FF"))) %v%
  Heatmap(mat,
