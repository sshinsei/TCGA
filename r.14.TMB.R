...
gene <- read.table("../03_venn/feature_genes.txt",sep = "\t",header = F,check.names = F)
...
#########summary##################  ??????
pt.vs.rt <- mafCompare(m1 = laml, m2 = laml2, m1Name = 'high', m2Name = 'low', minMut = 1)
print(pt.vs.rt)
pt.vs.rt$results <- pt.vs.rt$results[-(which(pt.vs.rt$results$or == Inf)),]
p <- forestPlot(mafCompareRes = pt.vs.rt, pVal = 0.05, color = c('royalblue', 'maroon'),
                geneFontSize = 1)
pdf("forestplot.pdf")
...
p <- ggviolin(tmb, x = "risk", y = "total_perMB_log", fill = "risk",
              ...
              add.params = list(fill="white"))+
  ylab("Tumor Burden Mutation")+
  stat_compare_means(label.x = 1.25)  #????ͼƬ?޸?
p
ggsave(filename = "TMB.tiff", plot = p, dpi = 400, width = 6, height = 5)
ggsave(filename = "TMB.pdf", plot = p, dpi = 500, width = 6, height = 5)

pdf(file="TMBplot.pdf",width=6,height=5)
ggplot(tmb, aes(x=risk,y = total_perMB_log,color = risk))+
  geom_violin(aes(fill=NULL),trim=FALSE)+
  ...
  labs(x = "" , y = "TMB")+ 
  stat_summary(fun.y=median, geom="point", size=3.5)+
  stat_compare_means(label.y = max(tmb$total_perMB_log)+0.5, label.x = 1.3, method = "wilcox.test")
theme_bw()
dev.off()
#####
...
# mafSurvival(maf = laml, genes = 'DNMT3A', time = 'days_to_last_followup', Status = 'Overall_Survival_Status', isTCGA = TRUE)
colnames(tmb_table_wt_log)[1] = "id"
tmb <- merge(tmb_table_wt_log[,c(1,4)],st,by = "id")
tmb$risk <- factor(tmb$risk,levels = c("low","high"))
pdf(file="TMBplot.pdf",width=5,height=5)
ggplot(tmb, aes(x=risk,y = total_perMB_log,color = risk))+
  geom_violin(aes(fill=NULL),trim=FALSE)+
  geom_beeswarm(cex = 2, priority = "descending", shape = 23) +
  scale_color_manual(values = c( "cornflowerblue","tomato")) +
