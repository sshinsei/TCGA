#############################-----------GEO---------------------#################
rm(list=ls())
setwd('E:/LZ/24080/09_GEO/01_multicox')

###------------merge data---------------
cli = read.table("../../00_rawdata/01_GEO/GSE76427.clinic.txt", header = T, sep = "\t", check.names = FALSE)
nor = read.csv("../../00_rawdata/01_GEO/GSE76427.txt", header = T, sep = "\t", check.names = FALSE,
               row.names = 1)
# 多因素cox后的结果的geneCoef
gene = read.table("../../06_multicox/multiCox.csv", header = T, sep = ",", check.names = FALSE)
...
#############################--------ROC----------###############################
rm(list=ls())
rt <- read.csv("E:/LZ/24080/09_GEO/01_multicox/risk.txt", 
...
library(survival)
library(ggsci)
rt = read.table("E:/LZ/24080/09_GEO/01_multicox/risk.txt", 
