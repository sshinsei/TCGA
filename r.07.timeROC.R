...
library(timeROC)
library(ggsci)
rt <- read.table("../00_rawdata/risk.txt", header = T, sep = "\t", check.names = F, row.names = 1)
...
hub_gene <- read.csv('../06_multicox/multiCox.csv')

library(tidyverse)
train_data <- read.csv("../02_DEG/normalizeExp.txt", 
                       sep = "\t", row.names=1)
...
if(plot == T){
  ggviolin(
    ...
    fill = "Group", 
    add = "boxplot",
    palette = c( "#7a9ac0", "#ef5675"),
    add.params = list(fill = "white"),
    #trim = T
    # add = "jitter",
    # ylim = c(4, 8.8)
  )+stat_pvalue_manual(stat_res,
                       y.position = c(12,12,12,12,12,12,15), # 有几个基因就设置几个值
                       size = 6,
                       color='red',
                       family = "Times",
                       label = "p.signif",
  ...
  ggsave(width=24,height=16,'boxplot.pdf')
  ggsave(width=24,height=16,'boxplot.png')
}
