...
library(survival)
rt = read.table("../00_rawdata/tumor_clinical.txt", header = T, sep = "\t",
                row.names = 1, check.names = F)
rt = rt[rt$futimes > 30, ] # 344
rt[ , "futimes"] = rt[ , "futimes"]/365
#3/7groups
...
#读取输入文件
# 0.05=54 0.01=20
rt <- read.table("univariateCox_P0.001.csv",header=T,sep=",",row.names=1,check.names=F)
