rm(list=ls())
setwd("E:/LZ/24080")
if (!dir.exists("16_drug")){dir.create("16_drug")}
setwd("16_drug")
library(pRRophetic)
library(edgeR)
library(tidyverse)
...
setwd("16_drug")
# 设置文件夹路径
folder_path <- "E:/LZ/24080/16_drug"
...
exp <- read.table("../00_rawdata/geneExp.txt", sep= "\t", header = T, row.names = 1,
                  check.names = F) #固定的文件
dim(exp)
# [1] 23808    60
exp[1:4, 1:4]
...
gene <- read.csv("../06_multicox/multiCox.csv") #your genes
...
#outTab1 <- outTab %>% filter(Drug %in% c('Cisplatin','Dasatinib',"Sunitinib",
#                                        "Imatinib","Paclitaxel","Gemcitabine")
#                             & Gene %in% genelist)
# lenvatinib:30,243
# dgidb:搜索获批药物
# CDX2: 1.TEGAFUR:80
# NEK2: 1.PAZOPANIB:188 2.PALBOCICLIB: 146
outTab <- outTab[c(30, 243, 80, 188, 146),]
#1.Scatter plot
plotList_1 <- list()
corPlotNum <- 5
...
ggsave(filename = "plotList_1.tiff", plot = p1, dpi = 500, width = 16, height = 12)
ggsave(filename = "plotList_1.pdf", plot = p1, dpi = 500, width = 16, height = 12)
