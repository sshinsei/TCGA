...
rm(list = ls())
setwd("E:/LZ/24105")
if (! dir.exists('./02_enrich')){
  dir.create('./02_enrich')
}
setwd('./02_enrich')
...
file_name <- "non-response"

#数据准备
#enrich_ID <- read.table('../04_unite_gene/1.14_unite_gene/02.final_common_gene.txt',header = T,check.names = F)#双模块
enrich_ID <- read.csv('../01_DEG/DEG_sig.csv', header = T) # 54
# enrich_ID <- clust1
gene <- enrich_ID[enrich_ID$change == "DOWN",]$X # 605
...
# [1] "得到289个结果，其中247个生物学过程，35个细胞组分，7个分子功能"

write.table(GO_ALL,file = paste0('00.GO_ALL.',file_name,'.txt'),sep = "\t",quote = F,row.names = F)
write.table(as.data.frame(BP), paste0('00.GO_BP.',file_name,'.txt'), sep = '\t', row.names = FALSE, quote = FALSE)
write.table(as.data.frame(CC), paste0('00.GO_CC.',file_name,'.txt'), sep = '\t', row.names = FALSE, quote = FALSE)
write.table(as.data.frame(MF), paste0('00.GO_MF.',file_name,'.txt'), sep = '\t', row.names = FALSE, quote = FALSE)


display_number = c(289, 35, 7)#这三个数字分别代表选取的BP、CC、MF的通路条数，这个自己设置就行了
...
#[1] "sno(s)RNA-containing ribonucleoprotein complex（）；box H/ACA RNP complex（）；Cajal body（）；nucleolar exosome (RNase complex)（）；nuclear exosome (RNase complex)（）；cytoplasmic exosome (RNase complex)（）；exosome (RNase complex)（）；exoribonuclease complex（）；telomerase holoenzyme complex（）；euchromatin（）；fibrillar center（）；exon-exon junction complex（）；eukaryotic translation initiation factor 4F complex（）；U2-type catalytic step 1 spliceosome（）；catalytic step 1 spliceosome（）；SAM complex（）；postsynaptic cytosol（）；MIB complex"
...
if(T){
  go_enrich_df <- data.frame(
    ID=c(ego_result_BP[1:10,]$ID, ego_result_CC[1:10,]$ID, ego_result_MF[1:10,]$ID),
    Description=c(ego_result_BP[1:10,]$Description, ego_result_CC[1:10,]$Description, ego_result_MF[1:10,]$Description),
    GeneNumber=c(ego_result_BP[1:10,]$Count, ego_result_CC[1:10,]$Count, ego_result_MF[1:10,]$Count),
    type=factor(c(rep("BP:biological process", 10), 
                  rep("CC:cellular component", 10),
                  rep("MF:molecular function", 10)), 
  ...
  ggsave(filename = paste0("01.go_barplot.",file_name,".pdf"), height = 8, width = 14, p)
  ggsave(filename = paste0("01.go_barplot.",file_name,".png"), height = 8, width = 14, p)
} 

# circ
if(F){
  select_DERNA <- read.csv("../01_DEG/DEG_sig.csv",row.names = 1)
  select_DERNA <- select_DERNA[rownames(select_DERNA) %in% gene,]
  ...
  go_tem <- rbind(ego_result_BP[1:7,],ego_result_CC[1:7,],ego_result_MF[1:7,])
...
save.image(paste0('GO.',file_name,'.Rdata'))
...
file_name <- "response"
enrich_ID<-read.csv('../01_DEG/DEG_sig.csv', header = T) # 54
gene <- enrich_ID[enrich_ID$change == "UP",]$X # 502
...
write.table(hh,file = paste0("00.KEGG.",file_name,".txt"),sep = "\t",quote = F,row.names = F)
rownames(hh) <- 1:nrow(hh)
hh$order=factor(rev(as.integer(rownames(hh))),labels = rev(hh$Description))
kk <- hh[c(1:10),] 
paste0(kk$Description,collapse = "（）；")
# "Cell cycle（）；HIF-1 signaling pathway（）；Cellular senescence（）；Motor proteins（）；Biosynthesis of amino acids（）；IL-17 signaling pathway（）；Efferocytosis（）；Rheumatoid arthritis（）；Carbon metabolism（）；Fluid shear stress and atherosclerosis"
...
if(T){
  ...
  ggsave(filename = paste0("02.KEGG_dot.",file_name,".pdf"), height = 8, width = 9, p2)
  ggsave(filename = paste0("02.KEGG_dot.",file_name,".png"), height = 8, width = 9, p2)
}
save.image(paste0('KEGG.',file_name,'.Rdata'))
