...
padj = 0.05
setwd('E:/LZ/24105')
if (! dir.exists('./14_high_low_enrich')){
  dir.create('./14_high_low_enrich')
}
setwd('./14_high_low_enrich')
library(edgeR)
library(tidyverse)
rt = read.csv("E:/LZ/24080/01_TCGA/files/mRNA_count.txt", sep="\t", header=T, check.names=F) 
...
x <- read.table("./normalizeExp_highlow.txt",sep="\t",header = T,row.names = 1,check.names = F)
x<- log2(x+1)
mat <- t(scale(t(x)))#归一化
# mat <- x
# mat <- 2^mat-1
...
#sep="\t")
group1 <- cluster1[,c(1,ncol(cluster1))]
...
mat <- as.matrix(mat)
heatmap_plot <- 
  HeatmapAnnotation(Group = group1$group, col = list(Group = c("high" = "#f94141", "low" = "#589ff3"))) %v%
  Heatmap(mat,
...
####富集数据文件准备####
#载入DESeq2差异分析结果表：
setwd('E:/LZ/24105/14_high_low_enrich')
if (! dir.exists('./all')){
  dir.create('./all')
}
setwd('./all')
...
# [1] "得到74个结果，其中24个生物学过程，14个细胞组分，36个分子功能"
...
display_number = c(24, 14, 36)#这三个数字分别代表选取的BP、CC、MF的通路条数，这个自己设置就行了
...
if(T){
  go_enrich_df <- data.frame(
    ID=c(ego_result_BP[1:20,]$ID, ego_result_CC$ID, ego_result_MF[1:20,]$ID),
    Description=c(ego_result_BP[1:20,]$Description,ego_result_CC$Description,ego_result_MF[1:20,]$Description),
    GeneNumber=c(ego_result_BP[1:20,]$Count, ego_result_CC$Count,ego_result_MF[1:20,]$Count),
    type=factor(c(rep("BP:biological process", 20), 
                  rep("CC:cellular component", display_number[2]),
                  rep("MF:molecular function", 20)), 
