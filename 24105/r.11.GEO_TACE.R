...
rm(list=ls())
setwd('E:/LZ/24105/')
if(! dir.exists("10_GEO_TACE")){dir.create("10_GEO_TACE")}
setwd("10_GEO_TACE")

###------------merge data---------------
cli = read.table("../00_rawdata/GSE14520/survival.csv", header = T, sep = ",")
nor = read.csv("../00_rawdata/GSE14520/GSE14520.txt", header = T, sep = "\t", check.names = FALSE,
               row.names = 1)
group <- read.csv("../00_rawdata/GSE14520/GSE14520.group.txt", header = T, sep = "\t", check.names = FALSE)
# 多因素cox后的结果的geneCoef
...
# gene <- read.csv('../06_lasso/geneCoef.txt',sep="\t",header = T)

# 因为都是肿瘤样本,case为接收TACE治疗的样本
case <- group[group$group == "Case",]$sample
nor <- nor[,colnames(nor) %in% case] # 105， 35+70
...
rt = read.csv("./multiInput.txt", 
              ...
              row.names = 1)
rt = rt[rt$futimes > 1, ]
rt[ , "futimes"] = rt[ , "futimes"]/12 #futime本身是日：除以365
...
library(timeROC)
library(ggsci)
rt <- read.csv("E:/LZ/24105/10_GEO_TACE/risk.txt", 
...
library(survival)
library(ggsci)
rt = read.table("E:/LZ/24105/10_GEO_TACE/risk.txt", 
