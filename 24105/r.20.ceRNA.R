rm(list = ls())
setwd("E:/LZ/24105")
if (!dir.exists("20_ceRNA")) {dir.create("20_ceRNA")}
setwd("20_ceRNA")

#利用r包筛选mirna
library(multiMiR)
hubgene <- read.csv('../07_multicox/multiCox.csv')
...
hubgene <- data.frame(symbol=targetscan2$miRNA)
...
write.csv(ce,'03.cerna_cytoscape.csv',row.names = F,quote = F)
# 81
...
mycol<- c4a('rainbow_wh_rd',39)
mycol2<- sample(mycol,length(mycol)) #随机打乱配色顺序
mycol2 <- scales::hue_pal()(nrow(df1))
