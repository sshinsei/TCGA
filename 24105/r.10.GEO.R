...
rm(list=ls())
setwd('E:/LZ/24105/')
if(! dir.exists("10_GEO")){dir.create("10_GEO")}
setwd("10_GEO")
...
nor = read.csv("../00_rawdata/GSE14520/GSE14520.txt", header = T, sep = "\t", check.names = FALSE,
               row.names = 1)
group <- read.csv("../00_rawdata/GSE14520/GSE14520.group2.txt", header = T, sep = "\t", check.names = FALSE)
# 多因素cox后的结果的geneCoef
...
library(timeROC)
library(ggsci)
rt <- read.csv("E:/LZ/24105/10_GEO/risk.txt", 
...
library(survival)
library(ggsci)
rt = read.table("E:/LZ/24105/10_GEO/risk.txt", 
...
type = c(rep("low", 121), rep("high", 121))
