...
rm(list = ls())
setwd('E:/LZ/24105') 
if(!dir.exists("03_model")){dir.create("03_model")}
setwd("03_model")
...
# 加载分组变量
group_list <- read.csv(file = "../00_rawdata/GSE14520/GSE14520.group.txt",
                       sep="\t")
group_list <- group_list %>% mutate(
  group = if_else(group == "tumor","Case","Normal")
)
Candidate_genes <- read.csv('../00_rawdata/genes.txt', header = F) # 13
# Candidate_genes <- read.csv('../04_DEG_clust/DEG_sig.csv', header = T) # 1461

# 提取样本子集和特征基因
expr_region <- exp %>% subset(., select = group_list$sample)
expr_top <- data.frame(t(expr_region[Candidate_genes$V1, ]))
