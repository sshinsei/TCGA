...
eset <- read.csv("../00_rawdata/normalizeExp.txt", 
                 sep = "\t", row.names=1)
colnames(eset) <- gsub("\\.","-",colnames(eset))
# 去正常样本
eset <- eset[,!grepl("11",colnames(eset))]
group <- read.csv(file = "../00_rawdata/group.txt",
                  sep="\t")
group <- group[group$sample %in% colnames(eset),]
# 加载表达矩阵数据
...
# --------------------------ssGSEA评分作为表形性状----------------------------------
if(T){
  score <- read.csv('../02_ssGSEA/01.ssgsea_score.csv',check.names = F,row.names = 1)
...
column = match("red", modNames1);
moduleGenes = moduleColors=="red";
red_kme<-kME[moduleGenes, ]
gene_red <- rownames(red_kme) %>% as.data.frame()
dim(gene_red)
# 869    1
...
if(T){
  ...
  moduleGenes = moduleColors=="red";
  red_keep = abs(kME[moduleGenes, column])>0.8&abs(geneTraitSignificance[moduleGenes, 1]) >0.2
  red_kme<-kME[moduleGenes, ]
  red_gene<-rownames(red_kme[red_keep,])
  write.table(rownames(red_kme),file = "07.red_all.txt",sep = "\t",quote = F,row.names = F,col.names=F)
  write.table(red_gene,file = "08.red_hub.txt",sep = "\t",quote = F,row.names = F,col.names=F)
}
