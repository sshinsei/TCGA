...
if (!dir.exists("./cibersort")) {dir.create("./cibersort")}
setwd("cibersort")
...
data<- read.csv("../../00_rawdata/normalizeExp.txt", 
                sep = "\t", row.names=1)
...
hubgene <- read.csv('../../07_multicox/multiCox.csv')$id
...
if(cor.map == 1){
  ssgsea_score <- readRDS("./epic_score.rds") %>% as.data.frame()
  library(tibble)
  library(magrittr)
  library(corrplot)
  library(reshape2)
  ...
  # sigcell --------------------------------------------------------------------=
  tiics_result <- readRDS("./epic_score.rds") %>% t %>% as.data.frame()
...
# "T cells CD8，T cells CD4 memory resting，T cells CD4 memory activated，T cells follicular helper，T cells regulatory (Tregs)，T cells gamma delta，Monocytes，Macrophages M0，Dendritic cells resting，Mast cells resting"
