rm(list=ls())
setwd("E:/LZ/24105/01_DEG")
if(!dir.exists("./GSE14520")){
  dir.create("./GSE14520")
}
setwd("GSE14520")
...
#exp <- read.csv("../00_rawdata/GSE122897/GSE122897.txt", 
#                  sep = "\t", row.names=1)
#rownames(exp) <- exp[,1]
#exp <- exp[,-1]
# 加载分组变量
group_list <- read.csv(file = "../../00_rawdata/GSE14520/GSE14520.group2.txt",
                       sep="\t")
...
# 调整差异比较的方向，Normal为-1
contrast.mat = makeContrasts(contrasts="Case-Normal", levels=design) #调整差异比较的方向，Normal为-1
contrast.mat
...
mat <- as.matrix(mat)
heatmap_plot <- 
  HeatmapAnnotation(Group = group1$group, col = list(Group = c("Case" = "#f94141", "Normal" = "#589ff3"))) %v%
  Heatmap(mat,
