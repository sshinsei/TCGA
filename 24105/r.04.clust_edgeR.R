...
library(tidyverse)
###-----------train------------
rt=read.table("../00_rawdata/mRNA_count.txt", sep="\t", header=T, check.names=F)  #change to your own file name_
colnames(rt)[2:ncol(rt)] <- gsub("\\.","-",colnames(rt)[2:ncol(rt)])
colnames(rt)[2:ncol(rt)] <- substring(colnames(rt)[2:ncol(rt)],1,16)

group_list <- read.csv("../02_clust/cluster.txt", sep="\t",header = F)
colnames(group_list) <- c("sample","group")
group_list$group <- paste0("clust",group_list$group)

id <- rt[,1]
rt <- rt[,colnames(rt) %in% group_list$sample]
rt <- cbind(data.frame(id = id),rt)
...
et <- exactTest(y,pair = c("clust1","clust2"))
...
# 373----1
# ---2
write.table(diffSig, file="diffSig.csv",sep="\t",quote=F)
